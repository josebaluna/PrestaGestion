...
class TPVApp:
    def __init__(self, root):
        ...
        self.config = Configuracion.obtener_sonido_activado(self)
        print(self.config.obtener_sonido_activado())
        if self.config.obtener_sonido_activado():
            pygame.mixer.init()
        ...
        # Crear widgets
        CTkLabel(root, text="Cliente:").grid(row=0, column=0)
        ...
        CTkLabel(root, text="EAN:").grid(row=1, column=0)
        ...
        btn_buscar_ean = CTkButton(root, text="Buscar", command=self.buscar_por_ean)
        btn_buscar_ean.grid(row=1, column=2)

        CTkLabel(root, text="Producto:").grid(row=2, column=0)
        ...
        CTkLabel(root, text="Cantidad:").grid(row=3, column=0)
        ...
        CTkButton(root, text="Agregar Producto", command=self.agregar_producto).grid(row=4, column=0, columnspan=2)
        CTkButton(root, text="Generar Factura", command=self.generar_factura).grid(row=5, column=0, columnspan=2)
        # Botón para generar factura
        CTkButton(root, text="Generar Factura", command=self.mostrar_datos_factura).grid(row=5, column=3, columnspan=2)

        button = CTkButton(root, text="Print Me", command=self.generar_factura)

        # Botón para imprimir el ticket
        self.boton_imprimir = CTkButton(root, text="Imprimir Ticket", command=self.imprimir_ticket)
        ...
        CTkLabel(root, text="Total:").grid(row=8, column=5)
        CTkLabel(root, textvariable=self.total).grid(row=8, column=6)
    ...
    def actualizar_base_de_datos(self):
        ...
        self.estado_actualizacion.configure(text=salida_actualizacion)
    ...
    def mostrar_ventana_actualizacion(self):
        ...
        self.estado_actualizacion = CTkLabel(self.ventana_actualizacion, text="Iniciando actualización...")
    ...
    def mostrar_imagen_producto(self, url_imagen):
        ...
        """imagen_widget = CTkLabel(self.frame_productos, image=imagen_tk)
    ...
    def mostrar_info_producto(self, event):
        ...
        self.label_nombre.configure(text=nombre_producto)

        # Mostrar la descripción del producto seleccionado
        self.label_precio.configure(text=precio_producto)

        # Mostrar la descripción del producto seleccionado
        # self.label_descripcion.configure(text=descripcion_producto)
    ...
    def crear_widgets_productos(self):
        ...
        for i, producto in enumerate(self.productos):
            ...
            boton_agregar = CTkButton(self.frame_productos, text="Agregar",
            ...
            nombre_producto = CTkLabel(self.frame_productos, text=producto["name"])
            nombre_producto.grid(row=i, column=2)

            precio_producto = CTkLabel(self.frame_productos, text=str(producto["price"]))
    ...
    def generar_factura(self):
        ...
        label = CTkLabel(root, text="Hey whatsup bro, i am doing something very interresting.")
    ...
    def mostrar_datos_factura(self):
        ...
        CTkLabel(self.ventana_factura, text="Datos del Ticket").grid(row=0, column=0, columnspan=2)
        ...
        CTkLabel(self.ventana_factura, text="Cliente:").grid(row=1, column=0, sticky="e")
        CTkLabel(self.ventana_factura, text=cliente).grid(row=1, column=1, sticky="w")
        CTkLabel(self.ventana_factura, text="Total:").grid(row=2, column=0, sticky="e")
        CTkLabel(self.ventana_factura, text=total).grid(row=2, column=1, sticky="w")
        CTkLabel(self.ventana_factura, text="Productos:").grid(row=3, column=0, sticky="e")
        for i, producto in enumerate(productos):
            CTkLabel(self.ventana_factura, text=f"{producto[1]} - Cantidad: {producto[2]} - Precio: {producto[3]}").grid(row=3+i, column=1, sticky="w")

        # Botones de Cobrar y Cancelar
        CTkButton(self.ventana_factura, text="Cobrar", command=self.cobrar).grid(row=4+len(productos), column=0, padx=5, pady=5)
        CTkButton(self.ventana_factura, text="Cancelar", command=self.ventana_factura.destroy).grid(row=4+len(productos), column=1, padx=5, pady=5)
    ...
    def limpiar_ventana_principal(self):
        ...
        self.label_nombre.configure(text="")  # Limpiar la etiqueta de nombre de producto
        self.label_precio.configure(text="")  # Limpiar la etiqueta de precio de producto
